...
def enable_dual_stack():
    """让服务器同时监听 IPv4 与 IPv6（双栈）。

    Werkzeug 在 IPv6 上默认不会接受 IPv4 连接（Windows 默认 IPV6_V6ONLY=1）。
    在绑定前把 IPV6_V6ONLY 设为 0，配合 host='::' 即可让同一个套接字同时
    接受 127.0.0.1（IPv4）与 ::1（IPv6），从而修复 localhost 解析到 ::1
    时资源加载失败的问题，同时保留 IPv4 可用性。
    """
    try:
        from werkzeug.serving import BaseWSGIServer
    except Exception:
        return
    original_bind = BaseWSGIServer.server_bind

    def dual_stack_server_bind(self):
    ...
    BaseWSGIServer.server_bind = dual_stack_server_bind
...
if __name__ == '__main__':
    ...
    enable_dual_stack()  # 让服务器同时监听 IPv4/IPv6（双栈）
    socketio.run(app, host='::', debug=False, port=open_port)
