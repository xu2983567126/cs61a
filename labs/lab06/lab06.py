...
class Transaction:
    ...
    def report(self) -> str:
        ...
        if self.changed():
            ...
            msg = '{0} {1}->{2}'.format(change, self.before, self.after)
...
class BankAccount:
    ...
    def __init__(self, account_holder: str, transactions: list[Transaction] = [], id: int = -1):
        self.balance: int = 0
    ...
    def deposit(self, amount: int) -> int:
        ...
        self.balance = getattr(transaction, "after")
        return self.balance
...
class Coin:
    ...
    def worth(self) -> int:
        "*** YOUR CODE HERE ***"
        assert self.cents, '面值不能为空'
        pass_year = Mint.present_year - self.year - 50
        worth_incre = pass_year if pass_year > 0 else 0
        return self.cents + worth_incre
...
class VirFib():
    ...
    VirFib object, value 8
    """
    def __init__(self, value: int = 0, prev_val = 1):
        self.value = value
        self.prev_val = prev_val

    def next(self):
        "*** YOUR CODE HERE ***"
        return VirFib(self.value + self.prev_val, self.value)
